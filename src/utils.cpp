...
void draw_rect(uint32_t* pixels, const size_t win_width, const size_t win_height, const size_t x, const size_t y, const size_t w, const size_t h, const uint32_t color)
{
    ...
    for(size_t i=y-radius; i<y+radius; i++)
    {
        for(size_t j=x-radius; j<x+radius; j++)
        {
            if(((j-x)*(j-x)+(i-y)*(i-y)) < radius*radius)
                pixels[j+i*win_width] = pack_rgb(100, 196, 106);
        }
    }
